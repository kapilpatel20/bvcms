...
(function($) {
    ...
    $.fn.columnFilter = function(options) {
        ...
        function fnCreateColumnSelect(oTable, aData, aids, iColumn, nTh, sLabel, bRegex) {
            if (aData == null)
            ...
            for (j = 0; j < iLen; j++) {
                if (typeof (aData[j]) != 'object') {
                    ...
                    if (escape(aData[j]) == currentFilter)
                        selected = 'selected '
                    r += '<option ' + selected + ' value="' + escape(aids[j]) + '">' + aData[j] + '</option>';
                }
                else {
                    var selected = '';
                    if (bRegex) {
                        ...
                        if (aData[j].value == currentFilter)
                            selected = 'selected ';
                        r += '<option ' + selected + 'value="' + aids[j].value + '">' + aData[j].label + '</option>';
                    } else {
                        if (escape(aData[j].value) == currentFilter)
                            selected = 'selected ';
                        r += '<option ' + selected + 'value="' + escape(aids[j].value) + '">' + aData[j].label + '</option>';
                    }
                }
            }
        ...
        function fnCreateSelect(oTable, aData, aids, bRegex) {
            ...
            fnCreateColumnSelect(oTable, aData, aids, _fnColumnIndex(i), th, label, bRegex); //Issue 37
                ...
                if (aoColumn != null) {
                    ...
                    switch (aoColumn.type) {
                        ...
                        case "select":
                            ...
                            fnCreateSelect(oTable, aoColumn.values, aoColumn.ids, aoColumn.bRegex);
                            break;
                        case "number-range":
